...
from web_crawler.web_scraper import WebScraper, is_local_url
    ...
    try:
        while True:
            try:
                ...
                if is_local_url(node.url):
                    logger.info(f"Skipping local URL {node.url}")
                    queue.task_done()
                    continue
